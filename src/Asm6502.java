...
public class Asm6502 {
    ...
    private static class Program {
        ...
        private static class AsmBlock {
            int start;
            ArrayList<Byte> asm = new ArrayList<>();
        ...
        static ArrayList<AsmBlock> asmBlocks = new ArrayList<>();
        // There may be a basic block provided by the writer.
        // This keeps us from clobbering theirs with our relocator.
        static boolean BASICBlock = false, MultiBlock = false;

        // Assembled program in current active block.
        static ArrayList<Byte> asm;
        ...
        private static void BuildProgram(String[] lines, FileOutputStream out) throws IOException {
            // Fixup OPS regexes.
            ...
            byte[] obj = assembleProgram(lines);
        ...
        private static void dumpSymtab(HashMap<String, String> symtab) {
            ...
            for (Map.Entry<String, String> entry : symtab.entrySet()) {
                ...
                System.out.printf("%-15s$%x(%d)%n", entry.getKey(), v, v);
            }
        }

        // Show symbol table contents.
        private static void dumpAsmBlocks(ArrayList<AsmBlock> asmBlocks) {
            System.out.println("\nAssembled Blocks:");
            for (AsmBlock entry : asmBlocks)
                System.out.printf("$%x (%d) length %d%n", entry.start, entry.start, entry.asm.size());
        }
        ...
        private static void preprocess(String[] prg, HashMap<String, String> symtab) {
            for (int x = 0; x < prg.length; x++) {
                ...
                // define statement
                if (line.matches("^(?i)define\\s+\\w+\\s+\\S+$")) {
        ...
        private static boolean assembleLine(String oline, int lineNum, HashMap<String, String> symtab) {
            ...
            if (line.matches("^\\*\\s*=\\s*\\$?\\w+$")) {
                ...
                int POS = tryParseWord(addr, null);
                for (AsmBlock ab : asmBlocks)
                    if (ab.start == POS) {
                        PC = POS;
                        START = POS;
                        // Only assign pointer if we're in phase 2.
                        if (lineNum != 0) asm = ab.asm;
                        return true;
                    }
                if (ERROR)
                    System.out.println("Line " + lineNum + ": " + oline + "\nBlock not found: " + addr + "(" + POS + ").");
                return false;
            }
        ...
        private static void collectLabels(String[] prg, HashMap<String, String> symtab) {
            ...
            for (int x = 0; x < prg.length; x++) {
                ...
                if (line.matches("^\\*\\s*=\\s*\\$?\\w+$")) {
                    String addr = strip(line.split("=")[1]);
                    // set vars for new block
                    START = tryParseWord(addr, null);
                    if (START == 2049) BASICBlock = true;
                    PC = START;
                    // add to block list
                    AsmBlock ab = new AsmBlock(START);
                    asmBlocks.add(ab);
                    // set adm pointer for assembled code - THIS WILL BE TOSSED FOR THIS PASS!
                    asm = new ArrayList<>();
                }
                // add label
                m = p.matcher(line);
                if (m.matches()) {
                    ...
                    if (!symtab.containsKey(key))
                        symtab.put(key, Integer.toString(PC));
                    else {
                        System.out.println("Line " + (x + 1) + ": " + line + "\nDuplicate label: " + m.group(1));
                        System.exit(2);
                    }
                }
        ...
        private static byte[] assembleProgram(String[] prg) {
            ...
            //System.out.println("PC = " + PC);
            dumpSymtab(symtab); //debug
            dumpAsmBlocks(asmBlocks); // debug
            //dumpAsm(); //debug
            int size = 0, start, idx = 0;
            /*
            Symbol table:
            ASMTABLE       $811(2065)
            SIZE           $fd(253)
             ...
             */
            // 158 = SYS, 153 = PRINT
            byte[] preamble = {(byte) 10, (byte) 8, (byte) 0, (byte) 0, (byte) 158, (byte) 50, (byte) 48, (byte) 54, (byte) 49, (byte) 0,
                    (byte) 0, (byte) 0, (byte) 76, (byte) 89, (byte) 8, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0,
                    ...
                    (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0,
                    (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0,
                    (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 0, (byte) 172, (byte) 16,
                    (byte) 8, (byte) 240, (byte) 44, (byte) 162, (byte) 255, (byte) 232, (byte) 189, (byte) 17, (byte) 8, (byte) 133,
                    (byte) 249, (byte) 232, (byte) 189, (byte) 17, (byte) 8, (byte) 133, (byte) 250, (byte) 232, (byte) 189, (byte) 17,
                    (byte) 8, (byte) 133, (byte) 251, (byte) 232, (byte) 189, (byte) 17, (byte) 8, (byte) 133, (byte) 252, (byte) 232,
                    (byte) 189, (byte) 17, (byte) 8, (byte) 133, (byte) 253, (byte) 232, (byte) 189, (byte) 17, (byte) 8, (byte) 133,
                    (byte) 254, (byte) 32, (byte) 139, (byte) 8, (byte) 136, (byte) 208, (byte) 214, (byte) 96, (byte) 72, (byte) 152,
                    (byte) 72, (byte) 138, (byte) 72, (byte) 166, (byte) 254, (byte) 24, (byte) 138, (byte) 101, (byte) 250, (byte) 133,
                    (byte) 250, (byte) 24, (byte) 138, (byte) 101, (byte) 252, (byte) 133, (byte) 252, (byte) 232, (byte) 164, (byte) 253,
                    (byte) 240, (byte) 14, (byte) 136, (byte) 240, (byte) 7, (byte) 177, (byte) 249, (byte) 145, (byte) 251, (byte) 136,
                    (byte) 208, (byte) 249, (byte) 177, (byte) 249, (byte) 145, (byte) 251, (byte) 136, (byte) 198, (byte) 250, (byte) 198,
                    (byte) 252, (byte) 202, (byte) 208, (byte) 237, (byte) 104, (byte) 170, (byte) 104, (byte) 168, (byte) 104, (byte) 96};

            if (asmBlocks.size() > 1) MultiBlock = true;
            ...
            int tbloffset = 18; // after load addr (2), BASIC(12), JMP(3)
            int blockstart = 0x8bf; // taken from the relocate code.

            ba[tbloffset - 1] = (byte) asmBlocks.size();
            for (x = 0; x < asmBlocks.size(); x++) {
                ...
                if (!BASICBlock) {
                    ...
                    ba[tbloffset++] = (byte) ((blockstart & 0xff00) >>> 8);
                    // set TO
                    ba[tbloffset++] = (byte) (ab.start & 0xff);
                    ba[tbloffset++] = (byte) ((ab.start & 0xff00) >>> 8);
                    // set SIZE
                    ba[tbloffset++] = (byte) (ab.asm.size() & 0xff);
                    ba[tbloffset++] = (byte) ((ab.asm.size() & 0xff00) >>> 8);

                    idx = blockstart-2047; // taken from the relocation code. $08bd - $0801
                    blockstart += ab.asm.size(); // prepare for next block
                }
    ...
    public static void main(String[] args) throws IOException {
        ...
        try {
            ...
            System.out.println("Cannot open output file. Terminating.");
            System.exit(2);
        }

        Program.BuildProgram(lines.toArray(new String[0]), out);
    }
}
